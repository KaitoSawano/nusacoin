...
static const uint32_t WALLET_NUX_KB_TO_NU_B = COIN / 1000; // 1 nu / B = 0.00001 NUX / kB
...
static void SetFeeEstimateMode(const CWallet* pwallet, CCoinControl& cc, const UniValue& estimate_mode, const UniValue& estimate_param)
{
    ...
    if (cc.m_fee_mode == FeeEstimateMode::NUX_KB || cc.m_fee_mode == FeeEstimateMode::NU_B) {
        ...
        if (cc.m_fee_mode == FeeEstimateMode::NU_B) {
            fee_rate /= WALLET_NUX_KB_TO_NU_B;
        }
...
static RPCHelpMan send()
{
    return RPCHelpMan{"send",
        ...
        RPCExamples{""
        "\nSend with a fee rate of 1 nusan per byte\n"
        + HelpExampleCli("send", "'{\"" + EXAMPLE_ADDRESS[0] + "\": 0.1}' 1 nu/b\n" +
