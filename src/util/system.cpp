...
const char * const BITCOIN_CONF_FILENAME = "nusacoin.conf";
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //nusacoin-tx using stdin
        std::string val;
...
{
    // Windows: C:\Users\Username\AppData\Roaming\Nusacoin
    // macOS: ~/Library/Application Support/Nusacoin
    // Unix-like: ~/.nusacoin
#ifdef WIN32
    // Windows
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/Nusacoin";
#else
    // Unix-like
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    // Make sure Nusacoin Core copyright is not removed by accident
    if (copyright_devs.find("Nusacoin Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + "The Nusacoin Core developers";
    }
