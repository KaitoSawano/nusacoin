...
static void SetupCliArgs(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-rpcwallet=<walletname>", "Send RPC for non-default wallet on RPC server (needs to exactly match corresponding -wallet option passed to nusacoind). This changes the RPC endpoint used, e.g. http://127.0.0.1:28832/wallet/<walletname>", ArgsManager::ALLOW_ANY, OptionsCategory::OPTIONS);
...
static int AppInitRPC(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs) || gArgs.IsArgSet("-version")) {
        ...
        if (!gArgs.IsArgSet("-version")) {
            strUsage += "\n"
                "Usage:  nusacoin-cli [options] <command> [params]  Send command to " PACKAGE_NAME "\n"
                "or:     nusacoin-cli [options] -named <command> [name=value]...  Send command to " PACKAGE_NAME " (with named arguments)\n"
                "or:     nusacoin-cli [options] help                List commands\n"
                "or:     nusacoin-cli [options] help <command>      Get help for a command\n";
...
public:
    ...
    UniValue ProcessReply(const UniValue& batch_in) override
    {
        ...
        if (networkinfo["version"].get_int() < 265000) {
        ...
        for (const UniValue& peer : batch[ID_PEERINFO]["result"].getValues()) {
            ...
            if (DetailsRequested()) {
                ...
                m_max_id_length = std::max(std::to_string(peer_id).length(), m_max_id_length);
        ...
        if (DetailsRequested() && !m_peers.empty()) {
            ...
            for (const Peer& peer : m_peers) {
                ...
                result += strprintf(
                    ...
                    peer.network,
                    peer.min_ping == -1 ? "" : std::to_string(round(1000 * peer.min_ping)),
                    peer.ping == -1 ? "" : std::to_string(round(1000 * peer.ping)),
                    peer.last_send == 0 ? "" : std::to_string(m_time_now - peer.last_send),
                    peer.last_recv == 0 ? "" : std::to_string(m_time_now - peer.last_recv),
                    peer.last_trxn == 0 ? "" : std::to_string((m_time_now - peer.last_trxn) / 60),
                    peer.last_blck == 0 ? "" : std::to_string((m_time_now - peer.last_blck) / 60),
                    peer.conn_time == 0 ? "" : std::to_string((m_time_now - peer.conn_time) / 60),
                    m_is_asmap_on ? 7 : 0, // variable spacing
                    m_is_asmap_on && peer.mapped_as != 0 ? std::to_string(peer.mapped_as) : "",
...
static UniValue CallRPC(BaseRequestHandler* rh, const std::string& strMethod, const std::vector<std::string>& args, const Optional<std::string>& rpcwallet = {})
{
    ...
    if (response.status == 0) {
        ...
        throw CConnectionFailed(strprintf("Could not connect to the server %s:%d%s\n\nMake sure the nusacoind server is running and that you are connecting to the correct RPC port.", host, port, responseErrorMessage));
...
static void ParseError(const UniValue& error, std::string& strPrint, int& nRet)
{
    if (error.isObject()) {
        ...
        if (err_code.isNum() && err_code.get_int() == RPC_WALLET_NOT_SPECIFIED) {
            strPrint += "\nTry adding \"-rpcwallet=<filename>\" option to nusacoin-cli command line.";
        }
    } else {
