...
The Nusacoin Core daemon can be configured to act as a trusted "border
router", implementing the nusacoin wire protocol and relay, making
...
The ZeroMQ feature in Nusacoin Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building nusacoind:
...
For instance:
    ...
    $ nusacoind -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubrawtx=ipc:///tmp/nusacoind.tx.raw \
...
These options can also be provided in nusacoin.conf.
...
From the perspective of nusacoind, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into nusacoind directly. Furthermore, no information is
...
using. Nusacoind appends an up-counting sequence number to each
